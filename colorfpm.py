...
if __name__ == "__main__":
    ...
    # elif color == "b":
    #     wavelength = 0.465  # um
    wavelengths = np.array([0.617, 0.5226, 0.465])
    ...
    for idx, color in enumerate(["g", "b"]):
        print(f"Loading {color} data")
        I = (
            sio.loadmat(
        ...
        Isum[idx + 1] = I[:, :, I_idx_use]

    # NA shift in pixel from different LED illuminations
    ledpos_true = np.zeros((3, ID_len, 2), dtype=int)
    count = 0
    for idx in range(ID_len):
        Fx1_temp = np.abs(
            Fxx1[None, ...].repeat(3, axis=0) - k0.reshape(-1, 1) * u_use[idx]
        )
        ledpos_true[:, count, 0] = np.argmin(Fx1_temp, axis=1)
        Fy1_temp = np.abs(
            Fyy1[None, ...].repeat(3, axis=0) - k0.reshape(-1, 1) * v_use[idx]
        )
        ledpos_true[:, count, 1] = np.argmin(Fy1_temp, axis=1)
        count += 1
    ...
    krr = krr[None, ...].repeat(3, axis=0)
    mask_k = k0.reshape(-1, 1, 1) ** 2 - krr**2 > 0
    kzz_ampli = mask_k * np.abs(
        np.sqrt((k0.reshape(-1, 1, 1) ** 2 - krr.astype("complex64") ** 2))
    )
    kzz_phase = np.angle(
        np.sqrt((k0.reshape(-1, 1, 1) ** 2 - krr.astype("complex64") ** 2))
    )
    ...
    Fxy2 = Fx2 + Fy2
    Pupil0 = np.zeros((3, M, N))
    Pupil0[Fxy2[None, ...].repeat(3, axis=0) <= (kmax.reshape(-1, 1, 1) ** 2)] = 1

    Pupil0 = (
        torch.from_numpy(Pupil0)
        .view(1, Pupil0.shape[0], Pupil0.shape[1], Pupil0.shape[2])
        .to(device)
    )
    ...
    for epoch in t:
        ...
        for color in torch.LongTensor([0, 1, 2]).to(device):
            cidx = color.detach().cpu().numpy()
            for it in range(ID_len // led_batch_size):  # + 1
                model.zero_grad()
                # TODO: Defocus mask depends on depth and wavelength
                dfmask = torch.exp(
                    1j
                    * kzz[0:1, cidx]
                    * dz[:, None, None].repeat(1, kzz.shape[2], kzz.shape[3])
                )
                led_num = led_idices[it * led_batch_size : (it + 1) * led_batch_size]
                dfmask = dfmask.unsqueeze(1).repeat(1, len(led_num), 1, 1)
                spectrum_mask_ampli = Pupil0[0:1, cidx : cidx + 1].repeat(
                    len(dz), len(led_num), 1, 1
                ) * torch.abs(dfmask)
                spectrum_mask_phase = Pupil0[0:1, cidx : cidx + 1].repeat(
                    len(dz), len(led_num), 1, 1
                ) * (
                ...
                with torch.cuda.amp.autocast(enabled=use_amp, dtype=torch.bfloat16):
                    img_ampli, img_phase = model_fn(color.unsqueeze(0))
                    img_complex = img_ampli * torch.exp(1j * img_phase)
                    uo, vo = (
                        ledpos_true[cidx, led_num, 0],
                        ledpos_true[cidx, led_num, 1],
                    )
                    x_0, x_1 = vo - M // 2, vo + M // 2
                    y_0, y_1 = uo - N // 2, uo + N // 2

                    oI_cap = torch.sqrt(Isum[cidx, :, :, led_num])
                    oI_cap = (
    ...
    save_path = os.path.join("trained_models", sample + ".pth")
